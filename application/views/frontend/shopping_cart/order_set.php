<!-- guest-login -->
<div class="col-md-9 col-sm-6 guest-login">
    <table class="table">
        <thead style="background-color: lightgrey;" border="1">
        <tr>
            <th>Image</th>
            <th>Product</th>
            <th>Price</th>
        ...
        <tr data-rowid="<?php echo $items['rowid']; ?>" >
            <td><a class="media-link" href="<?php echo $this->crud_model->product_link($items['id']); ?>">
                    <i class="fa fa-plus"></i>
                    <img src="<?php echo $items['image']; ?>" width="40" alt=""/>
                </a></td>
            <td> <a href="<?php echo $this->crud_model->product_link($items['id']); ?>">
                    <?php echo $items['name']; ?>
                </a>
                <hr class="mr0">
                <?php
            ...
            <td class="quantity pric"><?php echo currency($items['price']); ?></td>
            <td class="quantity product-single"><div class="cart-quantity">
                    <?php
                    if (!$this->crud_model->is_digital($items['id'])) {
                    ?>
                    <span class="buttons">
                    <div class="quant-input" style="width: 92px !important;">
                        <button type='button' class="btn in_xs quantity-button minus" value='minus'>
                            <i class="fa fa-minus"></i>
                        </button>
                        <input  type="number" class="form-control qty in_xs quantity-field quantity_field" data-rowid="<?php echo $items['rowid']; ?>" min="1"
                                max="<?php echo $row['current_stock']; ?>" data-limit='no' value="<?php echo $items['qty']; ?>" id='qty1' onblur="check_ok(this);" name='qty'
                                style="width: 100px;"/>
            ...
            <?php } ?></td>
            <td> <?php echo currency($items['subtotal']); ?> </td>
            <td><span class="close" style="color:#f00;">
...
<div class="col-md-12">
    <span class="btn btn-theme-dark btn-primary" onclick="load_address_form();">
