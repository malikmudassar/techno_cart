            ...
            <div style="left: 50%; transform: translateX(-50%);" class="col-md-8">
                <div class="card">
                    <div class="card-header">
                        <strong><span class="icon icon-plus-circle"></span> ADD BULK PRODUCT</strong>
                    </div>
                    <div class="card-body">
                        <div id="msg"></div>
                        <form id="bulk_product" class="form form-horizontal" style="margin-top: 30px" method="POST">
...
<script type="text/javascript">
    $(document).ready(function () {
        $("#bulk_product").on('submit', (function (e) {
            e.preventDefault();
            $("#msg").html('<div class="loading"></div>');
            var fd = new FormData(this);
            $.ajax({
                url: '<?php echo site_url($this->session->userdata['type']."/save_bulk_product") ?>',
                data: fd,
                type: "POST",
                contentType: false,
                processData: false,
